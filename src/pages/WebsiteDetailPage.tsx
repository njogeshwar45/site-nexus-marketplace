...
interface Website {
  ...
  preview_url: string | null; thumbnail_url: string | null; features: string[]; status: string;
}
...
const fadeUp = {
  ...
  visible: (i: number = 0) => ({ opacity: 1, y: 0, transition: { delay: i * 0.1, duration: 0.5, ease: [0.22, 1, 0.36, 1] } }),
};
...
export default function WebsiteDetailPage() {
  ...
  useEffect(() => { if (id) fetchWebsite(); }, [id]);
  ...
  const handleBuy = async () => {
    ...
    const { error } = await supabase.from('buy_requests').insert([{
      buyer_name: result.data.buyer_name, buyer_email: result.data.buyer_email,
      buyer_phone: result.data.buyer_phone || null, message: result.data.message || null,
      website_id: id, status: 'new',
    }]);
    setSubmitting(false);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-24 pb-16">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <motion.div className="lg:col-span-2" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}>
            ...
            {website.preview_url && (
              <a href={website.preview_url} target="_blank" rel="noopener noreferrer">
                <Button variant="outline" className="glass border-glass-border mb-8">Preview Site <ExternalLink className="ml-2 h-4 w-4" /></Button>
              </a>
            )}
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {website.tech_stack?.map((t, i) => (
                <motion.span key={t} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05, duration: 0.3 }} className="text-xs px-3 py-1 rounded-full bg-secondary/20 text-secondary">{t}</motion.span>
              ))}
            </div>
              ...
              <div className="mb-8">
                ...
                <ul className="space-y-2">
                  {website.features.map((f, i) => (
                    <motion.li key={f} variants={fadeUp} initial="hidden" animate="visible" custom={i} className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {/* Sidebar */}
          <motion.div className="space-y-6" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}>
            <GlassCard hover={false}>
              <p className="font-display text-4xl font-bold gradient-text mb-4">${website.price.toLocaleString()}</p>
              <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                <Button className="gradient-btn text-primary-foreground w-full py-6 text-lg rounded-xl border-0" onClick={() => setBuyOpen(true)}>Buy Now</Button>
              </motion.div>
            </GlassCard>
            <GlassCard hover={false}>
              ...
              <div className="space-y-3">
                {INCLUDES.map(({ icon: Icon, label }, i) => (
                  <motion.div key={label} variants={fadeUp} initial="hidden" animate="visible" custom={i} className="flex items-center gap-3 text-sm text-muted-foreground">
        ...
        {/* Related */}
        {related.length > 0 && (
          <motion.div className="mt-16" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <h2 className="font-display text-2xl font-bold mb-6">Related Websites</h2>
            <div className="grid md:grid-cols-3 gap-6">
              {related.map((w, i) => (
                <motion.div key={w.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }} whileHover={{ y: -6, transition: { duration: 0.25 } }}>
      ...
      <Dialog open={buyOpen} onOpenChange={setBuyOpen}>
        <DialogContent className="glass border-glass-border sm:max-w-md">
          {submitted ? (
            ...
            <>
              <DialogHeader><DialogTitle className="font-display text-xl">Purchase Enquiry</DialogTitle></DialogHeader>
